...
export default function HealthOfficerAuthForm() {
    ...
    const validateRegister = () => {
        ...
        // Required fields
        if (!r.officerName?.trim()) return 'Name is required';
        if (!r.officerId?.trim()) return 'Officer ID is required';
        if (!r.phone?.trim()) return 'Phone number is required';
        if (!r.region?.trim()) return 'Region is required';
    ...
    const onRegister = async (e) => {
        ...
        try {
            ...
            // Store health officer metadata in localStorage temporarily
            localStorage.setItem('pendingOfficerMetadata', JSON.stringify({
                officerName: String(registerForm.officerName).trim(),
                officerId: String(registerForm.officerId).trim(),
                phone: String(registerForm.phone).trim(),
                region: String(registerForm.region).trim(),
                role: 'health_officer',
            }));
            ...
            <div className="auth-panel">
                ...
                {mode === 'verify' ? (
                    ...
                    <form onSubmit={onRegister} className="auth-form">
                        <label>
                            Officer Name *
                            <input name="officerName" value={registerForm.officerName} onChange={handleRegisterChange} type="text" required />
                        </label>

                        <div className="two-col">
                            <label>
                                Officer ID *
                                <input name="officerId" value={registerForm.officerId} onChange={handleRegisterChange} type="text" required />
                            </label>
                            <label>
                                Phone *
                                <input name="phone" value={registerForm.phone} onChange={handleRegisterChange} type="text" required />
                            </label>
                        </div>

                        <label>
                            Region *
                            <input name="region" value={registerForm.region} onChange={handleRegisterChange} type="text" required />
                        </label>
